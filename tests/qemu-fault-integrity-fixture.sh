...
if [[ "$probe" == reserved-bit ]]; then
  ...
  printf -v expected_live_leaf '%u' \
    "$(( expected_leaf | (1 << 48) ))"
  record="LEANOS/14 PF-TERMINAL codec=1 case=reserved-bit vector=14 error=12 access=read cr2=${cr2} rip=user-a-reserved-fault-instruction expected-leaf=${expected_leaf} live-leaf=${expected_live_leaf} authorization=0 route=144115188075855874 halt=absorbing containment=0 cleanup=0 dispatch=0 return=none"
else
